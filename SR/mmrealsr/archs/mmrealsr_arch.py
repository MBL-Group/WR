...
class MMRRDBNet(nn.Module):
    ...
    def forward(self, x, custom_degrees=(None, None, None), anchor=None):
        ...
        feat = feat_res + feat
        # upsample
        feat = self.frelu(self.conv_up1(F.interpolate(feat, scale_factor=2, mode='nearest')))
        feat = self.frelu(self.conv_up2(F.interpolate(feat, scale_factor=2, mode='nearest')))
        out = self.conv_last(self.frelu(self.conv_hr(feat)))
        return out, degrees
